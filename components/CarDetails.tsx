...
const CarDetails = ({isOpen, closeModal, car}:CarDetailsProps) => {
  return (
    ...
    <Transition appear show={isOpen} as={Fragment}>
        <Dialog as='div' className="relative z-10 details" onClose={closeModal}>
            <Transition.Child
            as={Fragment} 
            ...
            <div className='fixed inset-0 overflow-y-auto '>
                <div className='flex min-h-full items-center justify-center p-4 text-center'>
                    ...
                    >
                        <Dialog.Panel className=" relative w-full max-w-lg max-h-[90vh] overflow-y-auto transform rounded-2xl bg-primary-gray-200  p-6 text-left shadow-xl transition-all flex flex-col gap-5">
                            <button type="button" onClick={closeModal} className='absolute top-2 right-2 z-10 w-fit p-2 bg-primary-gray-100 rounded-full'>
                            ...
                            <div className='flex-1 flex flex-col gap-2 '>
                                ...
                                <div className='mt-3 flex flex-wrap gap-4 '>
                                    {Object.entries(car).map(([key, value])=>(<div className='flex justify-between gap-5 w-full text-right' key={key}>
                                        ...
                                        <p className='text-white-100 font-semibold'>{value}</p>
                                        </div>))}
                                </div>
                            </div>
                        </Dialog.Panel>
